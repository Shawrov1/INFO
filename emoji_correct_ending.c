...
//placer le coup joué
int jouer_coup(int grille[lign][colo], int colonne, int jeton) {
...
void affichage_choix_rota(int tab_choix_rota[1]){
    ...
    if(tab_choix_rota[0]=='>'){
        ...
        printw("   à droite\n");
        attron(COLOR_PAIR(2));
        printw("%lc à gauche %lc",carreau,carreau);
        attroff(COLOR_PAIR(2));
    }
    //printw("%lc",carreau);
    //printw("prout")
...
void tab_copy(int tab[lign][colo],int tab2[lign][colo]){
    for (int i = 0; i < lign; i++) {
        for (int j = 0; j < colo; j++) {
            tab2[i][j] = tab[i][j];
        }
...
void win_add(int tab[lign][colo],int indices_gagnants[5][2],int jeton){
    for (int i = 0; i < 5; i++){
            tab[indices_gagnants[i][0]][indices_gagnants[i][1]]=jeton;   
        }
}

void animation_winner(int tab[lign][colo], int indices_gagnants[5][2],int jeton){

    for(int i=0;i<colo;i++){
        for(int j=0;j<lign;j++){
            tab[j][i] = ' ';
        }
        win_add(tab,indices_gagnants,jeton);
        clear();
        afficher_grille(tab);
        refresh();
        usleep(200000);
...
int main(){
    ...
    while (!gagnant) {
        ...
        if(jouer_coup(tableau,co_jeton,jeton)){
            ...
            tab_copy(tableau,tab_rota);

            while(play_choix_rota){
                affichage_rota(tab_rota,coo_x,coo_y,dim);
                printw("\n");
                ...
                if(coo_choix_rota!=0){
                    tab_copy(tableau,tab_rota);
                    tourner(tab_rota,coo_y,coo_x,coo_choix_rota,dim);
                }
                else{
                    tab_copy(tableau,tab_rota);
                }
            }
            tab_copy(tab_rota,tableau);
            // x,y inversé
    ...
    if(gagnant){
        printw("\nil y un gagnant\n");
        animation_winner(tableau,indices_gagnants,jeton);
        for (int i = 0; i < 5; i++){
